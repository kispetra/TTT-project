...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['createCharacter'])) {
    ...
    if (!$character_name || !$level || !$alignment || !$character_class || !$background || !$species) {
        $error_msg = "All character data is required.";
    } else {
        $query = "INSERT INTO characters (character_name, level, alignment, character_class, background, species, user_id)
                  VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $con->prepare($query);
        $stmt->bind_param("sissssi", $character_name, $level, $alignment, $character_class, $background, $species, $user_id);
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php
                $all_species = [
                    "Aasimar", "Dragoborn", "Dwarf", "Elf", "Gnome", "Goliath", "Halfling", 
                    "Human", "Orc", "Tiefling"
                ];
